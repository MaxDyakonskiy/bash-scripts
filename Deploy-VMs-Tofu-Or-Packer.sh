...
printf "Стенд будет создан на корпоративном Бресте, который доступен по адресу: https://brest.astralinux.ru.\n"
...
if [ $BRANCH_GOTO = goToTofu ]; then
  ...
  cp --remove-destination opentofu/deployBrest/main.tf ~/opentofu-termidesk/main.tf
  ...
  if [ ! -s brestLogin.tfvars ]; then
    ...
    read -p 'Введите логин вашей доменной УЗ от brest.astralinux.ru: ' BR_LOGIN
...
read -ep "Вы хотите запустить Ansible для настройки развёрнутых ВМ? " ANSIBLE_CHOISE
  case $ANSIBLE_CHOISE in
    'yes' ) ANSIBLE_CHOISE_LABEL=yes;;
    'no' ) ANSIBLE_CHOISE_LABEL=no;;
     *) printf "Недопустимый выбор\n" && exit 1;;
    esac

if [ $ANSIBLE_CHOISE_LABEL == 'yes' ]; then
  cd $BEGIN_DIR
  /bin/bash setVMs.sh
fi
